...
export default function MatrixLoader({ tipo = 'hackeo' }) {
  ...
  const [mensaje, setMensaje] = useState(t('ui.matrix.mensajes.' + tipo + '.0'))
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      setPct(progreso)
      setMensaje(t('ui.matrix.mensajes.' + tipo + '.' + (progreso >= 72 ? 2 : progreso >= 35 ? 1 : 0)))
  ...
  return (
    <div className="matrix-loader" role="status" aria-live="polite" aria-label={t('ui.matrix.aria')}>
      <div className="matrix-rain" aria-hidden="true">
        {columnas.map(col => <span key={col.id} style={{ animationDelay: col.delay }}>{col.texto}</span>)}
      </div>
      <div className="matrix-terminal">
        <div className="matrix-top mono"><span>{t('ui.matrix.prompt')}</span><b>{t('ui.matrix.canal')}</b></div>
        ...
        <div className="matrix-status mono">{t('ui.matrix.protocolo')}<span className="matrix-cursor">_</span></div>
      </div>
    </div>
  )
}